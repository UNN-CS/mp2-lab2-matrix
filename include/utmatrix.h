...
{
	if (pos<StartIndex||pos >= StartIndex + Size) throw - 1;
...
{
	if (Size != v.Size) throw - 1;
	ValType tmp = 0;//TVector tmp(Size, StartIndex);
	for (int i = 0; i < Size; ++i)
		tmp += pVector[i] * v.pVector[i];//tmp.pVector[i] = pVector[i] * v.pVector[i];
	return tmp;
} /*-------------------------------------------------------------------------*/
...
TMatrix<ValType> TMatrix<ValType>::operator+(const TMatrix<ValType> &mt)
{
	if (mt.Size != this->Size) throw - 1;
	return TVector<TVector<ValType>>(*this) + mt;//TVector<TVector<ValType>>::operator+mt;
} /*-------------------------------------------------------------------------*/
