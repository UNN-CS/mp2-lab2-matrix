...
public:
	...
	TVector(const TVector<ValType> &v);                // конструктор копирования
	~TVector();
	int GetSize() const { return this->Size; } // размер вектора
	int GetStartIndex() const { return this->StartIndex; } // индекс первого элемента
...
TVector<ValType>::TVector(int s, int si)
{
	if (s < 1 || s>MAX_VECTOR_SIZE)
		throw "Negative or null or over maximal size!";
	else
		this->Size = s;
	if (si < 0)
...
TVector<ValType>::TVector(const TVector<ValType> &v) :Size(v.GetSize()), StartIndex(v.GetStartIndex())
{
	try
	{
		this->pVector = new ValType[this->Size];
...
{
	if (pos<0 || pos>(this->Size + this->StartIndex - 1))
...
TVector<ValType>& TVector<ValType>::operator=(const TVector<ValType> &v)
{
	try
	{
		if (this == &v)
			return *this;
		this->Size = v.GetSize();
		this->StartIndex = v.GetStartIndex();
		delete[] this->pVector;
		this->pVector = new ValType[this->Size];
		for (int i = 0; i < this->Size; i++)
			this->pVector[i] = v.pVector[i];
